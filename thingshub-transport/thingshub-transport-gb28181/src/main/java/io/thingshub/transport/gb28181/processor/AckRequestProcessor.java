package io.thingshub.transport.gb28181.processor;

import io.thingshub.transport.BaseChannelContext;
import io.thingshub.transport.Processor;
import io.thingshub.transport.gb28181.message.AckRequest;
import lombok.NonNull;
import lombok.extern.slf4j.Slf4j;

/**
 * <p>
 * ACK请求处理器
 * </p>
 *
 * @author albert pi
 * @since 1.0.0
 */

@Slf4j(topic = "io.thingshub.transport.processor")
public class AckRequestProcessor extends Processor<BaseChannelContext, AckRequest> {

	@Override
	public void process(@NonNull BaseChannelContext ctx, @NonNull AckRequest request) {
		// TODO
	}

}
