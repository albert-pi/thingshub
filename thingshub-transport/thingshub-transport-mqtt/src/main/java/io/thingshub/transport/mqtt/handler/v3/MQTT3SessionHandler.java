...
public class MQTT3SessionHandler extends MQTTSessionHandler {
	...
	public MQTT3SessionHandler(String tenant, String clientId, int keepAlive, boolean sessionPresent, int sessionExpiryInterval, LastWillMessage lwm) {
		super(tenant, clientId, keepAlive, sessionPresent, sessionExpiryInterval, lwm);
	}

	@Override
	protected ProtocolValidationResult validatePubMessage(MqttPublishMessage message) {
		...
		if (!UTF8Util.isWellFormed(topic, false)) {
			...
			result = new ProtocolValidationResult(true, null);
		}
