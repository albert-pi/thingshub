...
public class LogQueryParser {
	...
	public static Query parse(String queryStr, int minTermFreq, int minDocFreq, IndexReader indexReader, Analyzer analyzer) {
	...
	private static Query getMoreLikeThisQuery(String queryStr, int minTermFreq, int minDocFreq, IndexReader indexReader, Analyzer analyzer) {
		...
		if (!multiField.isEmpty()) {
			...
			try {
				mutiFieldQuery = MultiFieldQueryParser.parse(multiValue.toArray(new String[multiValue.size()]),
						multiField.toArray(new String[multiField.size()]), mutilsOccur.toArray(new BooleanClause.Occur[mutilsOccur.size()]), analyzer);
