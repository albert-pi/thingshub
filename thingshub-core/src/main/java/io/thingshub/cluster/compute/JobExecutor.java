...
public class JobExecutor {
	...
	public void execute(RunnableJob job, boolean includeLocal) {
		ClusterGroup clusterGroup = includeLocal ? ignite.cluster() : ignite.cluster().forRemotes();
		if (CollUtil.isEmpty(clusterGroup.nodes())) {
			return;
		}
	...
	public void executeAsync(RunnableJob job, boolean includeLocal) {
		ClusterGroup clusterGroup = includeLocal ? ignite.cluster() : ignite.cluster().forRemotes();
		if (CollUtil.isEmpty(clusterGroup.nodes())) {
			return;
		}
