...
public final class TaskTracker {
	...
	public CompletableFuture<Void> whenComplete(BiConsumer<Void, Throwable> biConsumer) {
		return CompletableFuture.allOf(trackedFutures.toArray(new CompletableFuture[0])).whenComplete(biConsumer);
	}

	public CompletableFuture<Void> whenCompleteAsync(BiConsumer<Void, Throwable> biConsumer, Executor executor) {
		return CompletableFuture.allOf(trackedFutures.toArray(new CompletableFuture[0])).whenCompleteAsync(biConsumer, executor);
	}
