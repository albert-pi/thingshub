	...
	public boolean delete(String key) {
		TreeVisitor<T, Boolean> visitor = new TreeVisitor<T, Boolean>(Boolean.FALSE) {
			public void visit(String key, TreeNode<T> parent, TreeNode<T> node) {
				...
				if (result) {
					// If there no children of the node we need to delete it from the its parent children list
			...
			/**
			 * Merge a child into its parent node. Operation only valid if it is only child of the parent node and parent node is not a real node.
			 */
